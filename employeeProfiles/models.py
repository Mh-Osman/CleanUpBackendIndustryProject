    ...
    def save(self, *args, **kwargs):
        ...
        if EmployeeSalary.objects.filter(
            ...
            raise ValidationError({
                "month": f"⚠️ Salary for employee '{self.employee.name}' for {self.month.strftime('%B %Y')} already exists!"
            })

        # Use atomic to catch DB-level IntegrityError
        try:
            with transaction.atomic():
                super().save(*args, **kwargs)
        except IntegrityError:
            raise ValidationError({
                "month": f"⚠️ Salary for employee '{self.employee.name}' for {self.month.strftime('%B %Y')} already exists (DB constraint)!"
            })
    def __str__(self):
